...
class Component extends DCLogic {
  state = { screen: null, step: 1, ticket: 'Professional', day: 0, payMethod: 'M-Pesa', payStatus: 'processing' };

  get screen() { return this.state.screen ?? (this.props.startScreen || 'home'); }
  set(screen) { this.setState({ screen }); }

  renderVals() {
    const accent = this.props.accentColor ?? '#8ad356';
    const screen = this.screen;
    const go = (id) => () => this.set(id);

    const PUB = [['home','Home'],['events','Events'],['event','Event detail'],['speakers','Speakers'],['schedule','Schedule'],['register','Registration'],['checkout','Checkout'],['payment','Payment status'],['receipt','Receipt']];
    const USR = [['u-dash','Dashboard'],['u-ticket','My ticket'],['u-schedule','My schedule'],['u-network','Networking'],['u-notify','Notifications'],['u-cert','Certificates']];
    const ADM = [
      { title:'Overview', items:[['a-dash','Dashboard']] },
      { title:'Event management', items:[['a-events','Events'],['a-create','Create event'],['a-tickets','Tickets']] },
      { title:'Registration', items:[['a-attendees','Attendees'],['a-checkin','Check-in']] },
      { title:'Program', items:[['a-schedule','Schedule builder'],['a-timeline','Timeline']] },
      { title:'Marketing', items:[['a-poster','Poster designer'],['a-comms','Communications'],['a-sponsors','Sponsors']] },
      { title:'Finance & data', items:[['a-payments','Payments'],['a-reports','Reports']] }
    ];

    const group = screen.startsWith('u-') ? 'user' : screen.startsWith('a-') ? 'admin' : 'public';
    const is = {
      public: group === 'public', user: group === 'user', admin: group === 'admin',
      home: screen==='home', events: screen==='events', event: screen==='event',
      speakers: screen==='speakers', schedule: screen==='schedule', register: screen==='register',
      checkout: screen==='checkout', payment: screen==='payment', receipt: screen==='receipt',
      uDash: screen==='u-dash', uTicket: screen==='u-ticket', uSchedule: screen==='u-schedule',
      uNetwork: screen==='u-network', uNotify: screen==='u-notify', uCert: screen==='u-cert',
      aDash: screen==='a-dash', aEvents: screen==='a-events', aCreate: screen==='a-create',
      aTickets: screen==='a-tickets', aAttendees: screen==='a-attendees', aCheckin: screen==='a-checkin',
      aSchedule: screen==='a-schedule', aTimeline: screen==='a-timeline', aPoster: screen==='a-poster',
      aComms: screen==='a-comms', aSponsors: screen==='a-sponsors', aPayments: screen==='a-payments',
      aReports: screen==='a-reports'
    };

    const groupTabs = [['public','Public site','home'],['user','Attendee','u-dash'],['admin','Admin','a-dash']].map(([id,label,first]) => ({
      label, go: go(first),
      bg: group===id ? '#111510' : 'transparent',
      fg: group===id ? '#fbfaf0' : '#5b6349'
    }));

    const chipSource = group==='public' ? PUB : group==='user' ? USR : ADM.flatMap(s => s.items);
    ...
    const siteNav = [['home','Home'],['events','Events'],['speakers','Speakers'],['schedule','Schedule'],['event','About']].map(([id,label]) => ({
      label, go: go(id),
    ...
    const chosen = TICKETS.find(t => t.name === this.state.ticket) || TICKETS[1];
    ...
    return {
      accent,
      showNav: this.props.showPrototypeNav !== false,
      showCollage: this.props.showHeroCollage !== false,
      is, groupTabs, screenChips, siteNav, userNav, adminNav, dayTabs,
      dayItems: DAYS[day].items,

      goRegister: go('register'), goEvents: go('events'), goEvent: go('event'),
      goSpeakers: go('speakers'), goTicket: go('u-ticket'), goUserSchedule: go('u-schedule'),
      goCreate: go('a-create'),

      heroStats: [
        { value:'1,200+', label:'Attendees' }, { value:'50+', label:'Speakers' }, { value:'30+', label:'Sessions' }
      ],
      featureStats: [
        { value:'1,200+', label:'Attendees' }, { value:'50+', label:'Speakers' }, { value:'30+', label:'Sessions' }
      ],
      collageA: [
        { slot:'collage-1', art:'img/scene-1.png', label:'Keynote stage' },
        { slot:'collage-2', art:'img/scene-4.png', label:'BCI workshop' }
      ],
      collageB: [
        { slot:'collage-3', art:'img/scene-2.png', label:'Research showcase' },
        { slot:'collage-4', art:'img/scene-3.png', label:'Networking lounge' }
      ],
      categories: ['Neuroscience','AI & Brain Technology','Brain Computer Interface','Neurohealth','Research','Innovation','Investment'],
      whyAttend: [
      ...
      sponsorLogos: [
        { slot:'spo-1', art:'img/spo-1.png', name:'Vodacom Tanzania' },
        { slot:'spo-2', art:'img/spo-2.png', name:'MUHAS' },
        { slot:'spo-3', art:'img/spo-3.png', name:'CRDB Bank' },
        { slot:'spo-4', art:'img/spo-4.png', name:'COSTECH' },
        { slot:'spo-5', art:'img/spo-5.png', name:'Yas Tanzania' },
        { slot:'spo-6', art:'img/spo-6.png', name:'Ministry of Health' }
      ],
      footerCols: [
      ...
      featuredSpeakers: [
        { slot:'spk-1', art:'img/spk-1.png', initials:'NM', name:'Dr. Neema Mwakalinga', role:'Neuroscientist', org:'MUHAS, Dar es Salaam' },
        { slot:'spk-2', art:'img/spk-2.png', name:'Dr. Juma Kibwana', role:'BCI Researcher', org:'Muhimbili National Hospital' },
        { slot:'spk-3', art:'img/spk-3.png', name:'Prof. Grace Kimaro', role:'Neuro-ethics', org:'University of Dar es Salaam' },
        { slot:'spk-4', art:'img/spk-4.png', name:'Eng. Baraka Msangi', role:'ML Engineer', org:'NM-AIST, Arusha' }
      ],
      speakers: [
        { slot:'spk-1', art:'img/spk-1.png', initials:'NM', name:'Dr. Neema Mwakalinga', role:'Neuroscientist', org:'MUHAS, Dar es Salaam' },
        { slot:'spk-2', art:'img/spk-2.png', name:'Dr. Juma Kibwana', role:'BCI Researcher', org:'Muhimbili National Hospital' },
        { slot:'spk-3', art:'img/spk-3.png', name:'Prof. Grace Kimaro', role:'Neuro-ethics', org:'University of Dar es Salaam' },
        { slot:'spk-4', art:'img/spk-4.png', name:'Eng. Baraka Msangi', role:'ML Engineer', org:'NM-AIST, Arusha' },
        { slot:'spk-5', art:'img/spk-5.png', name:'Dr. Amina Salum', role:'Neurologist', org:'Mnazi Mmoja Hospital, Zanzibar' },
        { slot:'spk-6', art:'img/spk-6.png', initials:'EK', name:'Dr. Elias Kilonzo', role:'Biomedical Eng.', org:'KCMC, Moshi' },
        { slot:'spk-7', art:'img/spk-7.png', initials:'LM', name:'Dr. Lucy Mtei', role:'Public Health', org:'Ifakara Health Institute' },
        { slot:'spk-8', art:'img/spk-8.png', initials:'JN', name:'Joseph Nyerere', role:'Founder', org:'NeuroLab Tanzania' }
      ],
      speakerFilters: ['All tracks','Neuroscience','BCI','Neurohealth','Investment'],
      eventFilters: ['Category','Date','Location','Event type'],
      events: [
        { slot:'ev-1', art:'img/event-a.png', title:'NeuroTech Summit 2026', date:'20 Nov 2026', city:'Dar es Salaam', price:'From TZS 20,000', tag:'Summit' },
        { slot:'ev-2', art:'img/event-b.png', title:'BCI Hands-on Workshop', date:'10 Dec 2026', city:'Arusha', price:'TZS 60,000', tag:'Workshop' },
        { slot:'ev-3', art:'img/event-a.png', title:'AI in Healthcare Conference', date:'15 Jan 2027', city:'Dodoma', price:'TZS 80,000', tag:'Conference' },
        { slot:'ev-4', art:'img/event-b.png', title:'Neurohealth Clinical Forum', date:'02 Feb 2027', city:'Mwanza', price:'TZS 45,000', tag:'Forum' },
        { slot:'ev-5', art:'img/event-a.png', title:'Research Methods Bootcamp', date:'18 Feb 2027', city:'Online', price:'Free', tag:'Bootcamp' },
        { slot:'ev-6', art:'img/event-b.png', title:'Zanzibar Neuro Innovation Day', date:'05 Mar 2027', city:'Zanzibar', price:'Invite only', tag:'Investor' }
      ],
      highlights: ['Neuroscience','Artificial Intelligence','Brain Computer Interface','Healthcare','Startups'],
      faqs: ['Is the ticket transferable?','Do students get a discount?','Will sessions be recorded?','Is there airport transport?'],
      ...
      ticketOptions: TICKETS.map(t => ({
        ...t,
        pick: () => this.setState({ ticket: t.name }),
        border: this.state.ticket===t.name ? '#2f8f3c' : 'rgba(18,21,12,.1)',
        bg: this.state.ticket===t.name ? '#f4f8ea' : '#fbfaf0'
      })),
      payMethods: [
      ...
      stepNext: () => step===5 ? this.set('u-dash') : step===4 ? this.set('checkout') : this.setState({ step: Math.min(5, step+1) }),
      stepBack: () => this.setState({ step: Math.max(1, step-1) }),

      userStats: [
        { value:'01', label:'Upcoming events' }, { value:'03', label:'Saved sessions' },
        { value:'01', label:'Tickets' }, { value:'00', label:'Certificates' }
      ],
      interestChips: ['AI','BCI','Neuroscience','Investment'],
      attendeesNetwork: [
        { slot:'spk-1', art:'img/spk-1.png', initials:'NM', name:'Dr. Neema Mwakalinga', role:'Neuroscientist', org:'MUHAS, Dar es Salaam', tags:'AI · BCI · Research' },
        { slot:'spk-8', art:'img/spk-8.png', initials:'JN', name:'Joseph Nyerere', role:'Founder', org:'NeuroLab Tanzania', tags:'Startups · Hardware' },
        { initials:'AS', name:'Aisha Salim', role:'PhD Candidate', org:'University of Dodoma', tags:'Imaging · Research' },
        { slot:'spk-6', art:'img/spk-6.png', initials:'EK', name:'Dr. Elias Kilonzo', role:'Biomedical Eng.', org:'KCMC, Moshi', tags:'Devices · Clinical' },
        { initials:'GM', name:'Grace Mollel', role:'Investment Analyst', org:'Sahara Ventures', tags:'Investment · Health' },
        { slot:'spk-7', art:'img/spk-7.png', initials:'LM', name:'Dr. Lucy Mtei', role:'Public Health', org:'Ifakara Health Institute', tags:'Neurohealth · Policy' }
      ],
      notifications: [
      ...
      chartBars: ['Aug','Sep','Oct 1','Oct 2','Oct 3','Oct 4','Nov 1','Nov 2','Nov 3','Nov 4']
        .map((label,i) => ({ label, h: [22,34,41,52,48,63,71,78,88,96][i] + '%', color: i>7 ? '#111510' : accent })),
      recentRegs: [
      ...
      builderSteps: ['Basic information','Date & venue','Tickets','Speakers','Schedule','Registration','Poster','Publish'].map((label,i) => ({
        n: '0' + (i+1), label,
        bg: i===0 ? '#111510' : '#fff',
      ...
      lastScans: [
        { name:'Zainab Komba', time:'09:42' }, { name:'Bryan Kachocho', time:'09:41' },
        { name:'Neema Joseph', time:'09:39' }, { name:'Aisha Salim', time:'09:36' }
      ],
      paymentStats: [
        { label:'Revenue', value:'TZS 42.5M' }, { label:'Successful', value:'1,180' },
        { label:'Pending', value:'35' }, { label:'Failed', value:'30' }
      ],
      transactions: [
      ...
      sponsors: [
        { slot:'spo-1', art:'img/spo-1.png', name:'Vodacom Tanzania', level:'Title sponsor' },
        { slot:'spo-2', art:'img/spo-2.png', name:'MUHAS', level:'Platinum' },
        { slot:'spo-3', art:'img/spo-3.png', name:'CRDB Bank', level:'Platinum' },
        { slot:'spo-4', art:'img/spo-4.png', name:'COSTECH', level:'Gold' },
        { slot:'spo-5', art:'img/spo-5.png', name:'Yas Tanzania', level:'Silver' },
        { slot:'spo-6', art:'img/spo-6.png', name:'Ministry of Health', level:'Partner' }
      ],
      reportStats: [
        { label:'Registrations', value:'1,245' }, { label:'Paid', value:'1,180' },
        { label:'Attendance', value:'845' }, { label:'Revenue', value:'TZS 42.5M' }
      ],
      ticketSales: [
      ...
      goCheckout: go('checkout'), goReceipt: go('receipt'),
      payMethods2: [
        ...
        pick: () => this.setState({ payMethod: m.id }),
        bg: this.state.payMethod===m.id ? '#f4f8ea' : '#fbfaf0',
        border: this.state.payMethod===m.id ? '#2f8f3c' : 'rgba(18,21,12,.1)'
      })),
      payMethod: this.state.payMethod,
      isMomo: ['M-Pesa','Airtel Money','Mixx by Yas','HaloPesa'].indexOf(this.state.payMethod) > -1,
      orderLines: [
      ...
      payStatusTabs: [['processing','Processing'],['success','Success'],['failed','Failed']].map(([id,label]) => ({
        label, go: () => this.setState({ payStatus: id }),
        bg: this.state.payStatus===id ? '#111510' : '#fff',
        fg: this.state.payStatus===id ? '#fbfaf0' : '#5b6349',
        border: this.state.payStatus===id ? '#111510' : 'rgba(18,21,12,.12)'
      })),
      pay: { processing: this.state.payStatus==='processing', success: this.state.payStatus==='success', failed: this.state.payStatus==='failed' },
      startPayment: () => this.setState({ screen: 'payment', payStatus: 'processing' }),
      receiptLines: [
      ...
      countries: [
        { name:'Tanzania', count:'850' }, { name:'Kenya', count:'120' },
        { name:'Uganda', count:'85' }, { name:'Rwanda', count:'70' },
        { name:'Zambia', count:'62' }, { name:'Burundi', count:'58' }
      ]
    };
  }
}
